...
import { Calendar, Trash2, Target, ShieldCheck, Plus, CheckCircle2, AlertTriangle, Loader2, Edit2, Check, X } from 'lucide-react';
import type { FinancialYear, FinancialPeriod, Category } from '../types/database.types';

// Define the unified notification state
...
export default function FinancialSetup() {
  ...
  // Unified World-Class Modal State
    ...
    if (currentProfile) {
      ...
      if (yearRes.data) {
        ...
        if (yearRes.data.length > 0) {
           ...
           setCategoryTargets(yearRes.data[0].category_targets || {});
        }
  ...
  /**
   * Initializes a brand new year and auto-generates all 12 monthly periods for it.
   */
  const handleAddYear = async (e: React.FormEvent) => {
    ...
    if (error) {
      setNotification({ isOpen: true, type: 'ERROR', title: 'Initialization Failed', message: error.message });
      return;
    }
    ...
    setNotification({ isOpen: true, type: 'SUCCESS', title: 'Financial Year Created', message: `The system has successfully generated 12 monthly periods for ${newYear.year}.` });
    fetchSetupData();
  };

  /**
   * Inline updates the Base Approved Budget for an existing financial year.
   */
  const handleSaveYearBudget = async (yearId: string) => {
    ...
    if (error) {
      setNotification({ isOpen: true, type: 'ERROR', title: 'Update Failed', message: error.message });
      return;
    }
  ...
  /**
   * Saves the specific target budgets mapped to categories into the JSONB column of the selected year.
   */
  const handleSaveBudgetTargets = async () => {
    if (!selectedBudgetYear) return;
    setSavingTargets(true);
    try {
      // Clean up zeros before saving to keep JSON light
      ...
      setNotification({ isOpen: true, type: 'SUCCESS', title: 'Targets Synced', message: 'Category budget targets successfully mapped to the Dashboard tracker!' });
    } catch (err: any) {
  ...
  const handleTargetChange = (catId: string, val: string) => {
    ...
    setCategoryTargets(prev => ({ ...prev, [catId]: num }));
  };

  const handleResetPeriod = async () => {
    if (resetConfirmation !== 'RESET') {
      setNotification({ isOpen: true, type: 'WARNING', title: 'Confirmation Required', message: "Please type 'RESET' exactly to confirm data wipe." });
      return;
    }
    ...
    if (error) {
      setNotification({ isOpen: true, type: 'ERROR', title: 'Wipe Failed', message: error.message });
    } else {
      setResetConfirmation('');
      setNotification({ isOpen: true, type: 'SUCCESS', title: 'Period Reset Complete', message: 'All transactions and reconciliation data for this period have been completely wiped.' });
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300 relative">
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
          <div className="w-full max-w-md bg-white dark:bg-[#121212] border border-slate-200 dark:border-[#27272A] rounded-3xl p-8 shadow-2xl space-y-6 text-center animate-modal">
            ...
            {notification.type === 'ERROR' && (
              <div className="mx-auto h-16 w-16 bg-red-100 dark:bg-red-950/60 rounded-2xl flex items-center justify-center text-red-600 dark:text-red-400 animate-bounce">
                <X className="h-8 w-8" />
              </div>
            )}
            ...
            <button 
              ...
              className={`w-full py-3 text-white rounded-xl text-xs font-bold shadow-md transition-all ${
                notification.type === 'SUCCESS' ? 'bg-brand dark:bg-emerald-700 hover:bg-brand-dark' :
      ...
      <div className="grid grid-cols-3 gap-1.5 p-1.5 bg-slate-200/70 dark:bg-slate-900/80 rounded-xl w-full max-w-lg border border-slate-300 dark:border-slate-800 shadow-inner">
        <button onClick={() => setActiveTab('YEARS')} className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] sm:text-xs font-bold transition-all truncate ${activeTab === 'YEARS' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400'}`}>
          <Calendar className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">System Years</span>
        </button>
        <button onClick={() => setActiveTab('BUDGET')} className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] sm:text-xs font-bold transition-all truncate ${activeTab === 'BUDGET' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400'}`}>
          <Target className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">Budget Targets</span>
        </button>
        <button onClick={() => setActiveTab('RESET')} className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] sm:text-xs font-bold transition-all truncate ${activeTab === 'RESET' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400'}`}>
          <Trash2 className="h-3.5 w-3.5 shrink-0" /> <span className="truncate">Data Reset</span>
        </button>
      </div>
      ...
      {activeTab === 'YEARS' && (
        <div className="bento-card space-y-6 bg-white dark:bg-[#121212]">
          <div className="flex justify-between items-center">
            ...
            {!isAddingYear && (
              <button onClick={() => setIsAddingYear(true)} className="flex items-center gap-2 bg-brand dark:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-xs font-bold shadow-sm">
          ...
          {isAddingYear && (
            <form onSubmit={handleAddYear} className="bg-slate-50/50 dark:bg-[#1A1A1A] p-4 rounded-xl border border-slate-200 dark:border-[#27272A] flex flex-wrap items-end gap-4 animate-in fade-in">
              ...
              <div className="space-y-1.5 flex-1 min-w-[150px]">
                ...
                <input type="number" step="0.01" required className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white" value={newYear.budget} onChange={e => setNewYear({...newYear, budget: e.target.value})} />
              </div>
                  ...
                  <tr key={y.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                    ...
                    <td className="p-4 font-mono font-medium text-slate-700 dark:text-slate-300">
                      {editingYearId === y.id ? (
                        <input 
                          ...
                          step="0.01"
                          autoFocus
                          className="w-full max-w-[150px] rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 py-1.5 text-xs font-bold text-slate-900 dark:text-white" 
                          value={editBudgetAmount} 
                          onChange={(e) => setEditBudgetAmount(e.target.value)}
                          onKeyDown={(e) => e.key === 'Enter' && handleSaveYearBudget(y.id)}
                        />
                      ) : (
                    ...
                    <td className="p-4 text-right">
                      {editingYearId === y.id ? (
                        ...
                        <button 
                          onClick={() => { setEditingYearId(y.id); setEditBudgetAmount(y.approved_budget.toString()); }} 
        ...
        <div className="bento-card space-y-6 bg-white dark:bg-[#121212]">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Assign target amounts to specific categories (e.g. Tithes, Pledges). Tracked live on the Dashboard.</p>
            </div>
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <select 
                ...
                onChange={(e) => {
                  ...
                  setCategoryTargets(yr?.category_targets || {});
                }}
              >
                {years.map(y => <option key={y.id} value={y.id}>Year {y.year}</option>)}
              </select>
              <button 
                onClick={handleSaveBudgetTargets} disabled={savingTargets || !selectedBudgetYear} 
                className="flex items-center justify-center gap-2 bg-brand dark:bg-emerald-700 hover:bg-brand-dark text-white px-5 py-2.5 rounded-xl text-xs font-bold shadow-sm shrink-0 transition-colors"
              >
        ...
        <div className="bento-card border-red-200 dark:border-red-900/30 bg-red-50/30 dark:bg-red-950/10 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
            <div className="space-y-1.5">
              ...
              <select 
                className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-semibold text-slate-900 dark:text-white shadow-sm focus:border-red-500"
            ...
            <div className="space-y-1.5">
              ...
              <input 
                type="text" placeholder="Type RESET here" 
                className="w-full rounded-xl border border-red-200 dark:border-red-900/50 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-bold text-red-900 dark:text-red-300 shadow-sm focus:border-red-500"
                value={resetConfirmation} onChange={(e) => setResetConfirmation(e.target.value)}
              />
            </div>
            <div className="flex items-end">
              <button 
                onClick={handleResetPeriod} disabled={resetting || resetConfirmation !== 'RESET'}
