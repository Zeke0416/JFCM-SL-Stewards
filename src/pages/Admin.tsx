// ==========================================
// SYSTEM ADMIN PAGE
// Purpose: Manage system settings, users, and categories.
// ==========================================

import { useState, useEffect } from 'react';
import { supabase } from '../lib/supabase';
import { useAuth } from '../contexts/AuthContext';
import { Users, FolderTree, Plus, Edit2, Check, X, Trash2, KeyRound, CheckCircle2, Copy, Loader2, AlertCircle } from 'lucide-react';
...
export default function Admin() {
  ...
  const fetchAdminData = async () => {
    try {
      setLoading(true);
      // FIX: Used .maybeSingle() to prevent crash if backend lags
      const { data: currentProfile } = await supabase.from('profiles').select('church_id, role').eq('id', user?.id).maybeSingle();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-slate-200 dark:border-[#27272A] pb-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">System Administration</h1>
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mt-1">Manage institutional users and dynamic ComBud categories.</p>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-1.5 p-1.5 bg-slate-200/70 dark:bg-[#1A1A1A] rounded-xl w-full max-w-[300px] border border-slate-300 dark:border-[#27272A] shadow-inner">
      ...
      {activeTab === 'USERS' && (
        <div className="bento-card space-y-6 bg-white dark:bg-[#121212] shadow-xl border border-slate-200 dark:border-[#27272A] rounded-3xl">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">Authorized Personnel Management</h3>
            ...
            <form onSubmit={handleCreateUser} className="bg-slate-50/50 dark:bg-[#1A1A1A] p-5 rounded-2xl border border-slate-200 dark:border-[#27272A] space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="text" required placeholder="Ezekiel Cabusay" className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newUser.fullName} onChange={e => setNewUser({...newUser, fullName: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase">Email Address</label>
                  <input type="email" required placeholder="user@jfcm.org" className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase">Temporary Password</label>
                  <input type="text" required placeholder="Min 6 characters" className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newUser.tempPassword} onChange={e => setNewUser({...newUser, tempPassword: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="block text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase">System Role</label>
                  <select className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newUser.role} onChange={e => setNewUser({...newUser, role: e.target.value})}>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setIsAddingUser(false)} className="px-4 py-2 text-xs font-bold text-slate-600 bg-white dark:bg-[#121212] border border-slate-200 dark:border-slate-700 rounded-xl">Cancel</button>
                <button type="submit" disabled={userLoading} className="px-5 py-2 text-xs font-bold text-white bg-brand dark:bg-emerald-700 rounded-xl shadow-sm flex items-center gap-2 hover:bg-brand-dark">
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
          <form onSubmit={handleManualPasswordReset} className="w-full max-w-md bg-white dark:bg-[#121212] border border-slate-200 dark:border-[#27272A] rounded-3xl p-8 shadow-2xl space-y-6 animate-modal">
            ...
            <div className="space-y-1.5">
              ...
              <input type="text" required placeholder="Min 6 characters" className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-3.5 py-3 text-xs font-medium text-slate-900 dark:text-white focus:border-amber-500" value={newTempPassword} onChange={e => setNewTempPassword(e.target.value)} />
            </div>
            <div className="flex gap-3 pt-2">
              <button type="button" onClick={() => { setIsResettingPassword(false); setResetTargetUser(null); }} className="flex-1 py-3 rounded-xl text-xs font-bold text-slate-600 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 transition-colors">Cancel</button>
        ...
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
          <div className="w-full max-w-md bg-white dark:bg-[#121212] border border-slate-200 dark:border-[#27272A] rounded-3xl p-8 shadow-2xl space-y-6 animate-modal text-center">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={() => setCreatedUserResult(null)} className="flex-1 py-3 rounded-xl text-xs font-bold text-white bg-brand dark:bg-emerald-700 hover:bg-brand-dark shadow-md transition-colors">Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'CATEGORIES' && (
        <div className="bento-card space-y-6 bg-white dark:bg-[#121212] shadow-xl border border-slate-200 dark:border-[#27272A] rounded-3xl">
          <div className="flex justify-between items-center">
            ...
            {!isAddingCat && (
              <button onClick={() => setIsAddingCat(true)} className="flex items-center gap-2 bg-brand dark:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-xs font-bold hover:bg-brand-dark transition-colors shadow-sm">
          ...
          {isAddingCat && (
            <form onSubmit={handleAddCategory} className="bg-slate-50/50 dark:bg-[#1A1A1A] p-4 rounded-xl border border-slate-200 dark:border-[#27272A] flex flex-wrap items-end gap-4 animate-in fade-in">
              <div className="space-y-1.5 flex-1 min-w-[120px]">
                ...
                <select className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newCat.type} onChange={e => setNewCat({...newCat, type: e.target.value as 'INCOME' | 'EXPENSE'})}>
              ...
              <div className="space-y-1.5 flex-1 min-w-[120px]">
                ...
                <input type="text" required placeholder="e.g. 501" className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newCat.code} onChange={e => setNewCat({...newCat, code: e.target.value})} />
              </div>
              <div className="space-y-1.5 flex-[2] min-w-[200px]">
                ...
                <input type="text" required placeholder="e.g. Honorarium" className="w-full rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-[#121212] px-3.5 py-2.5 text-xs font-medium text-slate-900 dark:text-white focus:border-brand" value={newCat.name} onChange={e => setNewCat({...newCat, name: e.target.value})} />
              </div>
          ...
          <div className="max-h-[500px] overflow-y-auto custom-scrollbar rounded-xl border border-slate-200 dark:border-[#27272A]">
            <table className="w-full text-left text-xs border-collapse relative">
              <thead className="sticky top-0 bg-slate-50 dark:bg-[#0A0A0A] z-10 border-b border-slate-200 dark:border-[#27272A] text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
                <tr>
