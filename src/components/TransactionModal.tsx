...
// Purpose: Modal form for creating and updating ledger entries with unassigned support.
// ==========================================
...
export default function TransactionModal({ isOpen, onClose, onSuccess, churchId, userId, initialData, defaultPeriodId }: TransactionModalProps) {
  ...
  /**
   * Resets and populates form state when modal opens or initialData changes.
   * Purpose: Prepares modal for creation or editing.
   */
  useEffect(() => {
    if (isOpen) {
      ...
      if (initialData) {
        ...
        setCategoryId(initialData.category_id || 'unassigned');
  ...
  /**
   * Fetches categories including an 'Unassigned / For Review' virtual option and financial periods.
   * Purpose: Populates dropdown selections for transaction entry.
   */
  const fetchMetadata = async () => {
    const { data: catData } = await supabase
      ...
      .eq('type', initialData ? initialData.type : type)
      .order('sort_order');
    
    // Virtual Unassigned category option
    const unassignedCategory: Category = {
      id: 'unassigned',
      church_id: churchId,
      type: type,
      export_code: '???',
      name: 'Unassigned / For Review',
      sort_order: 0,
    ...
    if (!initialData) {
      ...
      setCategorySearch('[???] Unassigned / For Review');
    } else if (initialData) {
      // FIX: Safely handle null category_id for unassigned transactions
      const targetCatId = initialData.category_id || 'unassigned';
      const found = combinedCategories.find(c => c.id === targetCatId);
      
      if (found) {
        setCategorySearch(`[${found.export_code}] ${found.name}`);
  ...
  /**
   * Validates and submits transaction payload to Supabase with strict 2-decimal rounding.
   * Purpose: Saves secure financial ledger entries.
   */
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (useBreakdown && breakdownItems.length > 0) {
      const breakdownText = breakdownItems.map(i => `${i.name}: ₱${Number(parseFloat(i.amount) || 0).toFixed(2)}`).join(', ');
      finalRemarks = finalRemarks ? `${finalRemarks} [Breakdown: ${breakdownText}]` : `[Breakdown: ${breakdownText}]`;
    }

    const payload = {
      church_id: churchId, 
      financial_period_id: selectedPeriodId, 
      ...
      <div className="w-full max-w-2xl bg-white dark:bg-[#121212] rounded-3xl shadow-2xl relative max-h-[95vh] flex flex-col p-0 overflow-hidden border border-slate-200 dark:border-[#27272A] animate-modal">
        {successAnim ? (
          <div className="p-16 flex flex-col items-center justify-center space-y-4 text-center my-auto">
            ...
            <p className="text-xs text-slate-500">Ledger synchronized and audit trail updated.</p>
          </div>
        ) : (
          <>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-5">
              {error && (
                <div className="bg-red-50 dark:bg-red-950/40 text-red-700 p-4 rounded-2xl text-xs font-medium flex items-center gap-2.5 border border-red-200">
              ...
              <form id="tx-form" onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <select required className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-3 text-xs font-bold text-slate-900 dark:text-white" value={selectedPeriodId} onChange={(e) => setSelectedPeriodId(e.target.value)}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="date" required className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-3 text-xs font-bold text-slate-900 dark:text-white" value={date} onChange={(e) => setDate(e.target.value)} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-5 relative">
                    ...
                    {isCategoryOpen && (
                      <div className="absolute z-20 mt-1 w-full bg-white dark:bg-[#121212] shadow-2xl max-h-60 rounded-2xl py-1 text-xs overflow-auto border border-slate-200 dark:border-[#27272A] ring-1 ring-black/5 custom-scrollbar">
                        {filteredCategories.map((c) => (
                          <div key={c.id} className={`px-4 py-3 cursor-pointer flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 ${categoryId === c.id ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-800 font-bold' : 'text-slate-700 dark:text-slate-300'}`} onClick={() => { setCategoryId(c.id); setCategorySearch(`[${c.export_code}] ${c.name}`); setIsCategoryOpen(false); }}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="text" placeholder="e.g. 1024" className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-3 text-xs font-bold text-slate-900 dark:text-white" value={receiptNo} onChange={(e) => setReceiptNo(e.target.value)} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" placeholder={getPayeePlaceholder()} className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-3 text-xs font-bold text-slate-900 dark:text-white" value={payeeName} onChange={(e) => setPayeeName(e.target.value)} />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input type="text" placeholder="Additional details..." className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-4 py-3 text-xs font-bold text-slate-900 dark:text-white" value={remarks} onChange={(e) => setRemarks(e.target.value)} />
                  </div>
                </div>
                  ...
                  <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-800 pb-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">Break down supplier sub-items before posting total.</p>
                      </div>
                    </div>
                  ...
                  {useBreakdown ? (
                    <div className="space-y-3 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                      {breakdownItems.map((item, index) => (
                        <div key={index} className="flex gap-2 items-center w-full">
                          <input type="text" placeholder="Item name" required className="flex-1 min-w-0 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-xs font-bold text-slate-900 dark:text-white" value={item.name} onChange={e => { const newItems = [...breakdownItems]; newItems[index].name = e.target.value; setBreakdownItems(newItems); }} />
                          <input type="number" step="0.01" placeholder="₱ Amount" required className="w-24 sm:w-32 min-w-0 shrink-0 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-2 text-xs font-bold text-slate-900 dark:text-white" value={item.amount} onChange={e => { const newItems = [...breakdownItems]; newItems[index].amount = e.target.value; setBreakdownItems(newItems); }} />
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <button type="button" onClick={() => setStrictMode(!strictMode)} className="text-[11px] font-bold text-slate-500 hover:text-white flex items-center gap-1.5">
                      ...
                      {strictMode ? (
                        <div className="grid grid-cols-3 gap-3">
                          <div className="col-span-2 relative">
                            ...
                            <input type="text" required placeholder="5000" className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm font-bold text-slate-900 dark:text-white" value={wholeAmount} onChange={(e) => setWholeAmount(e.target.value.replace(/[^0-9]/g, ''))} />
                          </div>
                          <div className="relative">
                            ...
                            <input type="text" maxLength={2} placeholder="00" className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm font-bold text-slate-900 dark:text-white text-center" value={decimalAmount} onChange={(e) => setDecimalAmount(e.target.value.replace(/[^0-9]/g, ''))} />
                          </div>
                        </div>
                      ) : (
                        <input type="number" step="0.01" required placeholder="0.00" className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm font-bold text-slate-900 dark:text-white" value={standardAmount} onChange={(e) => setStandardAmount(e.target.value)} />
                      )}
                    </div>
                  )}
                </div>
